...
# Therefore, we call all those perturbed directions types and define a set ttype below containing all the perturbed directions and their opposite directions.
...
# The ttype plus is eactly half of the ttype set. We divide the directions in the ttype set into two subsets. One is type_plus and the other one is type_minus.
# In the following constraints, we will use the relation that D_{-u}^t(n) = D_u^t(n-u).
# Moreover, we put all directions like (1,0,0) (1,1,0) in the set type_plus.
# If, for example (1,-1) and (-1,1) are in the ttype set, we put any one of them in set type_plus.
...
def init_c(model,*args):
    ...
    return flow_instance.c.__getitem__(tuple(fine_component+u+d+v)) #float(flow_instance.c.__getitem__(tuple(fine_component+u+d+v)))

model.c = Param(model.fine_comps_type_plus_state, model.ttype, initialize=init_c)
...
model.directed_expr = Expression(model.fine_partcoords, model.ttype, model.dstring, rule=init_directed_expr)



# Generate expressions FF(n), G(n), Fbar(n)
...
model.expr = Expression(model.fine_partcoords, model.ustring, rule=init_expr)
...
model.directed_shifted_expr = Expression(model.fine_coords_type_plus_state, model.ttype, model.dstring, rule=init_directed_shifted_expr)
...
model.shifted_expr = Expression(model.fine_partcoords, model.type_plus, model.ustring, rule=init_shifted_expr)
...
model.bound_bias_bd = Expression(model.fine_partcoords, model.type_plus, model.bound, rule=init_bound_bias_bd)
...
model.bound_bias = Expression(model.type_plus, model.fine_comps_unbddim, model.bound, rule=init_bound_bias)
...
model.bound_error_bd = Expression(model.fine_partcoords, model.bound, rule=init_bound_error_bd)
...
model.bound_error = Expression(model.fine_comps_unbddim, model.bound, rule=init_bound_error)
