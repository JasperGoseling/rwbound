...
# M and N are the capacity for the first and the second node repectively
# T is the threshold for service rate change
M = 10
N = M
T = 4
...
# model.Fbar = Param(model.comps, model.extdimset, initialize=init_FF)
...
def init_pi_bar(model,*args):
    component = [item for item in args[0:dimen]]
    dim = args[dimen]
    return pi_bar(component,dim)

model.pi_bar = Param(model.comps, model.dimset, initialize=init_pi_bar)

def init_comp_weight(model,*args):
    component = [item for item in args[0:dimen]]
    if component[0] <= T:
        result = 1
    else:
...
model.comp_weight = Param(model.comps, initialize=init_comp_weight)
    ...
    for component in model.comps:
        ...
        for dim in model.dimset:
            if dim in bddim:
                ...
                if model.pi_bar.__getitem__(component+tuple([dim])) == 1:
                    result_part = result_part*(end - start + 1)
                else:
                    result_part = result_part*(model.pi_bar.__getitem__(component+tuple([dim]))**(end+1)-model.pi_bar.__getitem__(component+tuple([dim]))**start)/ (model.pi_bar.__getitem__(component+tuple([dim]))-1)
            else:
                start = component[dim]
                result_part = result_part*model.pi_bar.__getitem__(component+tuple([dim]))**start/(1-model.pi_bar.__getitem__(component+tuple([dim])))
        result += model.comp_weight.__getitem__(component)*result_part
    return 1/result
            ...
            for dim in model.dimset:
                ...
                if dim == c_dim:
                    ...
                    if dim in bddim:
                        ...
                        if model.pi_bar.__getitem__(component+tuple([dim])) == 1:
                            result_part = result_part*(end-start+1)*(start+end)/2
                        else:
                            result_part = result_part*( model.pi_bar.__getitem__(component+tuple([dim]))*(model.pi_bar.__getitem__(component+tuple([dim]))**start + end*model.pi_bar.__getitem__(component+tuple([dim]))**(end+1) - (end+1)*model.pi_bar.__getitem__(component+tuple([dim]))**end) - start*(model.pi_bar.__getitem__(component+tuple([dim]))-1)*model.pi_bar.__getitem__(component+tuple([dim]))**start )/ (model.pi_bar.__getitem__(component+tuple([dim]))-1)**2
                    else:
                        start = component[dim]
                        # Formula for \sum_{start_k}^{\infty} n_k rho^{n_k}
                        result_part = result_part*(model.pi_bar.__getitem__(component+tuple([dim]))**start*(start*(-model.pi_bar.__getitem__(component+tuple([dim])))+start+model.pi_bar.__getitem__(component+tuple([dim]))))/(model.pi_bar.__getitem__(component+tuple([dim]))-1)**2
                else:
                    if dim in bddim:
                        start = component[dim]
                        index = model.cuts[dim].value.index(component[dim])
                        ...
                        result_part = result_part*model.pi_bar.__getitem__(component+tuple([dim]))**start/(1-model.pi_bar.__getitem__(component+tuple([dim])))
            result += model.comp_weight.__getitem__(component)*(model.Fbar.__getitem__(component+tuple([c_dim])) + model.G.__getitem__(component+tuple([c_dim])) )*result_part
    # Multiply the summation by the normalization constant
    return model.const*result
...
# for fine_component in model.fine_comps:
#     component = TtoC(fine_component,tuple([0]*dimen))
#     for u in transitions_init(component):
#         if u in model.type_plus:
#             print(fine_component, u, instance.bound_bias_bd[fine_component+u].value)
