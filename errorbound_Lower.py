...
optimizer_flow = SolverFactory('gurobi')
optimizer = SolverFactory('gurobi')
...
# pi_bar = [((-0.3+math.sqrt(0.3**2+4*0.2*0.3))/(2*0.3),(-0.3+math.sqrt(0.3**2+4*0.2*0.3))/(2*0.3))]
rho = 0.1
pi_bar = [(rho,rho,rho)]
# pi_bar = [(1/2,1/2)]
...
flow_results = optimizer_flow.solve(flow_instance)
#print(flow_results)
...
model.ustring = Set(initialize=["Fbar","G","FF"])
...
def init_partcoords(model):
    ...
    for component in partition:
        ...
        for dim in nonsingulardims:
            if model.cuts[dim].value[component[dim][0]+1]-1 not in coordstmp:
...
#                 if  transition[dim] > 0:
#                     ind[dim] = 1
#             if ind != [0]*dimen and model.q.__getitem__(component+transition)!= 0:
#                 list.append(transition)
#     list_new = []
#     for item in list:
#         list_new.append(item)
...
def init_fine_coords_transition_plus_state(model):
...
model.fine_coords_transition_plus_state = Set(dimen=3*dimen, initialize=init_fine_coords_transition_plus_state)
...
def init_c(model,*args):
    ...
    if float(flow_instance.c.__getitem__(tuple(fine_component+u+d+v))) <= 1e-10 and float(flow_instance.c.__getitem__(tuple(fine_component+u+d+v))) >= -1e-10:
        return 0
    else:
        return float(flow_instance.c.__getitem__(tuple(fine_component+u+d+v)))
    # return float(flow_instance.c.__getitem__(tuple(fine_component+u+d+v)))

model.c = Param(model.fine_comps_transition_plus_state, model.type, initialize=init_c)
...
model.A = Var(model.comps, model.transitions, model.extdimset, domain=NonNegativeReals)
model.B = Var(model.comps, model.transitions, model.extdimset, domain=NonNegativeReals)
                ...
                for dim in model.dimset:
                    ...
                    if dim == c_dim:
                        ...
                        if dim in bddim:
                            ...
                            result_part = result_part*(pi_bar[i][dim]**start*(-pi_bar[i][dim]*start+start+pi_bar[i][dim]))/(pi_bar[i][dim]-1)**2
                    else:
                        if dim in bddim:
                            start = component[dim]
                            index = model.cuts[dim].value.index(component[dim])
                            ...
                            result_part = result_part*pi_bar[i][dim]**start/(1-pi_bar[i][dim])
                result += (model.Fbar.__getitem__(component+tuple([c_dim])) - model.G.__getitem__(component+tuple([c_dim])) )*result_part
        sum_result += weight[i]*result
...
################################################################################
#
# First define expressions for directed expression (A_u(n), B_u(n)) and
# undirected expression (Fbar(n),F(n), G(n))
#
################################################################################

# Define that D_u^t(n) = D_u^t(n) if u is trantiion_plus, and D_u^t(n) = D^t_{-u}(n+u) if u is not transition_plus.
...
def init_directed_expr(model,*args):
    ...
    if tuple(u) in transitions_init(component) and u != [0]*dimen:
        if tuple(u) in model.transition_plus:
...
model.directed_expr = Expression(model.fine_partcoords, model.transitions, model.dstring, initialize=init_directed_expr)
...
model.directed_shifted_expr = Expression(model.fine_coords_transition_plus_state, model.type_plus, model.dstring, initialize=init_directed_shifted_expr)
...
model.shifted_expr = Expression(model.fine_partcoords, model.transition_plus, model.ustring, initialize=init_shifted_expr)
...
def init_bound_bias_bd(model,*args):
    ...
    if tuple(u) in transitions:
        complete_transitions = func_fine_comps_transition_plus_state(fine_component,u)
            ...
            for d in complete_transitions:
                ...
                for v in model.type_plus:
                    if v in transitions_init(new_component):
                        if model.c.__getitem__(fine_component+tuple(u)+d+v) >= 0:
...
model.bound_bias_bd = Expression(model.fine_partcoords, model.transition_plus, model.bound, initialize=init_bound_bias_bd)
    ...
    if tuple(u) in transitions:
        ...
        if bound == "lower":
            for d in complete_transitions:
                ...
                for v in model.type_plus:
                    if v in transitions_init(new_component):
                        if model.c.__getitem__(tuple(fine_component+u)+d+v) >= 0:
...
model.bound_bias = Expression(model.transition_plus, model.fine_comps_unbddim, model.bound, initialize=init_bound_bias)
        ...
        for u in transitions:
            if u in model.transition_plus:
                ...
                if model.q.__getitem__(tuple(component)+u) >= 0:
                    ...
                    result += -model.q.__getitem__(tuple(component)+u)*model.directed_expr.__getitem__(tuple(coord)+u+tuple(["A"]))
        return -result - model.expr.__getitem__(tuple(coord+["G"]))
    if bound == "upper":
        for u in transitions:
            if u in model.transition_plus:
                if model.q.__getitem__(tuple(component)+u) >= 0:
...
def init_bound_error(model,*args):
    ...
    if bound == "lower":
        ...
        return -result - model.G.__getitem__(component+tuple([unbddim]))
    if bound == "upper":
        for u in transitions:
            if u in model.transition_plus:
                if model.q.__getitem__(tuple(component)+u) >= 0:
...
model.Constr_bound_bias_bd = Constraint(model.fine_partcoords, model.transition_plus, model.bound, rule=init_constr_bound_bias_bd)
...
model.Constr_bound_bias = Constraint(model.transition_plus, model.fine_comps_unbddim, model.bound, rule=init_constr_bound_bias)
